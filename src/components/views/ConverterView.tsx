...
export const ConverterView: React.FC = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        // Check for network error vs. server error
        if (response.status === 0) {
           throw new Error("Could not connect to the backend server. Is it running? (run 'npm run server')");
        }
      ...
      setFiles([]);
       ...
       if (err instanceof TypeError && err.message === 'Failed to fetch') {
         setError("Network Error: Could not connect to the backend server. Please make sure it's running by using the 'npm run server' command in your terminal.");
       } else {
